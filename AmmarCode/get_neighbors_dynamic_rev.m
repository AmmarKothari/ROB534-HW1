...
% Modified by Ammar Kothari, 2017

% Return the neighbors of a given state when moving from goal to start.
% MAP is the map of the environment
...
function [nbrs, num_nbrs] = get_neighbors_dynamic_rev(map, stateID)
...
for i = 1:size(acts,1)
    ...
    if (~check_hit(map, x, y, dx+ax, dy+ay))
        nbrs = [nbrs; dynamic_index_from_state(map,x+dx+ax,y+dy+ay,abs(dx+ax),abs(dy+ay))];
        num_nbrs = num_nbrs + 1;
    end
end
